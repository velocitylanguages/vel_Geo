"""
Velocity Georgian - Unified Social Media Upload Script
...
def get_latest_reel():
    ...
    return {
        ...
        "metadata": metadata,
        "category": metadata.get("category_english", "Georgian Learning"),
...
def generate_caption(phrases, category, platform="facebook"):
    ...
    if platform == "facebook":
        ...
        caption_lines = [
            f"🇬🇪 Learn Georgian with Velocity Georgian! 🇬🇪",
            f"",
            f"📚 Category: {category}",
            f"",
            f"🎯 Master Georgian one phrase at a time! Today's {category} lesson:",
            f""
        ]
        ...
        for i, phrase in enumerate(phrases[:5], 0):
            ...
            caption_lines.append(f"   📍 {phrase['georgian']}")
        ...
        caption_lines.extend([
            ...
            f"🔔 Follow for daily Georgian lessons!",
            f"",
...
def upload_to_all_platforms(video_path, caption, category):
    ...
    print("\n" + "="*80)
    print("🚀 VELOCITY GEORGIAN - MULTI-PLATFORM UPLOAD")
    print("="*80)
...
def main():
    ...
    print("\n" + "="*80)
    print("🇬🇪 VELOCITY GEORGIAN - AUTOMATED UPLOAD 🇬🇪")
    print("="*80)

    reel = get_latest_reel()

    if not reel:
        print("\n❌ No reel found! Run georgian_reels_automation.py first.")
        sys.exit(1)
